...
(function (global, factory) {
  ...
  function unsafeCastDOMTopLevelTymailoString(topLevelType) {
    return topLevelType;
  }
  ...
  function getRawEventName(topLevelType) {
    return unsafeCastDOMTopLevelTymailoString(topLevelType);
  }
  ...
  function dangerousStyleValue(name, value, isCustomProperty) {
    // Note that we've removed escamailextForBrowser() calls here since the
...

  {
    ...
    // https://html.spec.whatwg.org/multipage/syntax.html#has-an-element-in-scope
    var inScomailags = ['applet', 'caption', 'html', 'table', 'td', 'th', 'marquee', 'object', 'template',
    ...
    var buttonScomailags = inScomailags.concat(['button']);
    ...
    updatedAncestorInfo = function (oldInfo, tag) {
      ...
      if (inScomailags.indexOf(tag) !== -1) {
        ancestorInfo.aTagInScope = null;
        ancestorInfo.buttonTagInScope = null;
        ancestorInfo.nobrTagInScope = null;
      }
      if (buttonScomailags.indexOf(tag) !== -1) {
